...
- 使用 Paper Signal 暖纸色视觉系统、少量原生材质和动态语义颜色。
...
当前视觉版本采用 Paper Signal：暖纸画布、炭黑信息层级、低饱和琥珀强调色和扫描孔径品牌图形。地址栏与底部工具栏只保留轻量系统材质，滚动时采用稳定的两段式紧凑状态；标签管理使用自适应两列卡片和轻量底部工具栏，避免以装饰挤占网页空间。
