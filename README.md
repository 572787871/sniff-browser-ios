...
0.6.0 将资源嗅探改为按标签主动开启，并接入真实图片缩略图、普通文件下载、原生 Swift HLS 分片下载与顺序合并、持久化下载任务和文件库。应用不提供 DRM、直播流或访问控制绕过。
...
- 普通未加密 HLS VOD 会解析 Master/Media Playlist，下载初始化片段与媒体分片并按清单顺序合并；fMP4 输出 `.mp4`，MPEG-TS 输出 `.ts`。
...
- HLS 分片下载只支持有限、未加密、未受 DRM 保护的 VOD；直播、AES 加密和 DRM/FairPlay 明确拒绝。fMP4 仅做初始化片段与媒体片段的有序拼接，不做转码。
