...
0.6.2 将资源嗅探改为按标签主动开启，并接入真实图片缩略图、普通文件下载、原生 Swift HLS 分片下载、标准 AES-128 解密与顺序合并、持久化下载任务和文件库。应用不提供 DRM、直播流或访问控制绕过。
...
- 普通 HLS VOD 会解析 Master/Media Playlist，下载初始化片段与媒体分片并按清单顺序合并；标准 identity-key `AES-128` 会按清单密钥和 IV 解密，fMP4 输出 `.mp4`，MPEG-TS 输出 `.ts`。
