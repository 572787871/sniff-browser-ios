...
- 真实本地收藏持久化、URL 去重、搜索、打开、分享、复制和可撤销删除。
- 历史记录真实本地持久化、短时间重复访问合并、搜索、按日期分组和清理。
- 单条收藏与历史删除统一为 5 秒可撤销反馈；批量清空和本地文件删除继续使用确认保护。
...
- 资源分类与排序按标签会话保留；下载筛选、文件分类与排序、内容拦截统计范围作为非敏感展示偏好跨启动保留。
- 内容拦截、规则导入、网站白名单，以及摄像头/麦克风/位置的默认与按站点权限管理。
...
1. 在 iPhone 上回归标签连续转场、普通/无痕分页、网页主题色、键盘与小屏布局。
2. 按真实站点继续校准主动资源识别、封面缓存、在线播放、普通文件恢复和 HLS 分片检查点。
3. 完善文件夹、批量管理、缓存容量控制和浏览数据清理闭环。
4. 通过现有原生下载层与正式构建内置的 libav Media Pipeline，继续强化合法 HLS VOD 的 Variant 选择、分片字节统计与媒体兼容，不扩大到 DRM 或访问控制绕过。
5. 配置真实认证后端后再开放 Supabase Auth、Sign in with Apple 与同步。
6. 完成全量无障碍、性能、隐私清单、签名和 App Store 上线准备。
...
- [产品能力矩阵与统一交互规则](CAPABILITY_MATRIX.md)
