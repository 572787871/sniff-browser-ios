...
- 使用 Paper Signal 暖纸色视觉系统、五种全局主题色、少量原生材质和动态语义颜色。
...
当前视觉版本采用 Paper Signal：暖纸画布、炭黑信息层级、五种可选低饱和强调色和扫描孔径品牌图形。地址栏与底部工具栏只保留轻量系统材质，滚动时采用稳定的两段式紧凑状态；标签管理使用自适应两列卡片和轻量底部工具栏，避免以装饰挤占网页空间。
...
- 竖屏两列两行标签管理、普通/无痕原生左右分页、原生 Bottom Sheet 更多菜单和紧凑暖纸材质浏览器控件。
