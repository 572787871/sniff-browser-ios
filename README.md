...
0.6.5 将资源嗅探改为按标签主动开启，并接入真实图片缩略图、普通文件下载、原生 Swift 视频分片下载、标准 AES-128 解密、顺序合并与 MPEG-TS 到 MPEG-4 的本地转码兜底、持久化下载任务和文件库。下载任务会保存网页提供的真实视频封面，并在本地成品可抽帧后使用真实视频首帧。应用不提供 DRM、直播流或访问控制绕过。
...
- 普通 HLS VOD 会解析 Master/Media Playlist，下载初始化片段与媒体分片并按清单顺序合并；标准 identity-key `AES-128` 会按清单密钥和 IV 解密，fMP4 直接输出 `.mp4`，MPEG-TS 通过 AVFoundation 快速导出或逐轨无损重封装为本地可播放的 `.mp4`。
...
- HLS 分片下载支持有限、未受 DRM 保护的 VOD，以及清单公开声明的标准 identity-key `AES-128`；直播、`SAMPLE-AES`、非 identity KeyFormat 和 DRM/FairPlay 明确拒绝。fMP4 仅做初始化片段与媒体片段的有序拼接，不做转码。
