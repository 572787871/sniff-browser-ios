...
final class HLSAssetDownloadService: NSObject {
    ...
    private func finalizeMergedMedia(
        ...
        guard playlist.requiresTransportStreamExport else {
            guard await isPlayableVideo(at: mergedURL) else {
        ...
        // A long HLS program is not necessarily one continuous transport
        // stream. Concatenating its TS payloads can leave repeated PAT/PMT
        // tables and timestamp discontinuities that AVAssetExportSession may
        // reject. Build a continuous composition from the already decrypted
        // local segments first, preserving each segment's A/V timing, and
        // export that composition as a regular MPEG-4 file.
        do {
            if try await exportTransportStreamSegments(
                count: segmentCount,
                in: workDirectory,
                to: outputURL
            ) {
            ...
            // Fall through to the joined-stream compatibility paths below.
        }
    ...
    private func exportTransportStreamSegments(
        count: Int,
        in directory: URL,
        to outputURL: URL
    ) async throws -> Bool {
        ...
        var videoDestination: AVMutableCompositionTrack?
        var audioDestination: AVMutableCompositionTrack?
        var segmentStart = CMTime.zero
        var sourceAssets: [AVURLAsset] = []
        sourceAssets.reserveCapacity(count)
        var insertedVideo = false

        for index in 0..<count {
            try Task.checkCancellation()
            let segmentURL = segmentFileURL(in: directory, index: index)
            ...
            guard videoTrack != nil || audioTrack != nil else { return false }

            let videoRange: CMTimeRange?
            if let videoTrack {
                videoRange = try await videoTrack.load(.timeRange)
            } else {
                videoRange = nil
            }
            let audioRange: CMTimeRange?
            if let audioTrack {
                audioRange = try await audioTrack.load(.timeRange)
            } else {
                audioRange = nil
            }
            let validRanges = [videoRange, audioRange].compactMap { range -> CMTimeRange? in
                guard let range,
                      range.isValid,
            ...
            var sourceStart = validRanges[0].start
            var segmentDuration = validRanges[0].duration
            for range in validRanges.dropFirst() {
                if CMTimeCompare(range.start, sourceStart) < 0 {
                    sourceStart = range.start
                }
                if CMTimeCompare(range.duration, segmentDuration) > 0 {
                    segmentDuration = range.duration
                }
            }
            guard segmentDuration.isValid,
                  segmentDuration.isNumeric,
                  CMTimeCompare(segmentDuration, .zero) > 0
            else { return false }

            if let videoTrack, let videoRange {
                if videoDestination == nil {
                    videoDestination = composition.addMutableTrack(
                        withMediaType: .video,
                        preferredTrackID: kCMPersistentTrackID_Invalid
                    )
                    videoDestination?.preferredTransform = try await videoTrack.load(
                        .preferredTransform
                    )
                }
                guard let videoDestination else { return false }
                let relativeStart = CMTimeSubtract(videoRange.start, sourceStart)
                try videoDestination.insertTimeRange(
                    videoRange,
                    of: videoTrack,
                    at: CMTimeAdd(segmentStart, relativeStart)
                )
                insertedVideo = true
            }

            if let audioTrack, let audioRange {
                if audioDestination == nil {
                    audioDestination = composition.addMutableTrack(
                        withMediaType: .audio,
                        preferredTrackID: kCMPersistentTrackID_Invalid
                    )
                }
                guard let audioDestination else { return false }
                let relativeStart = CMTimeSubtract(audioRange.start, sourceStart)
            ...
            segmentStart = CMTimeAdd(segmentStart, segmentDuration)
        }

        // Keep the segment assets alive until the composition is fully built.
        _ = sourceAssets
        guard insertedVideo,
              segmentStart.isValid,
              segmentStart.isNumeric,
              CMTimeCompare(segmentStart, .zero) > 0
        else { return false }

        return await exportComposition(composition, to: outputURL)
    }
    ...
    private func exportComposition(
        ...
        for preset in [AVAssetExportPresetPassthrough, AVAssetExportPresetHighestQuality] {
            ...
            if await export(exporter), await isPlayableVideo(at: outputURL) {
                return true
            }
        }
        return false
    ...
    private func segmentFileURL(in directory: URL, index: Int) -> URL {
        // The payload has already been decrypted before it reaches this URL.
        // A media extension lets AVURLAsset inspect each local segment instead
        // of treating it as an opaque checkpoint file.
        directory.appendingPathComponent(String(format: "%08d.ts", index))
    }
