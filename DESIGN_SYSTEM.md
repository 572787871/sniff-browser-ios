...
第二轮把“内容优先”落实为可度量的浏览器框架：地址栏 46 点、底部工具栏 54 点（均不包含系统安全区域），控件浮于网页上方但不持续改变网页布局；标签、设置和管理页面继续使用同一套材质、间距与文字层级。
...
- 地址栏与浏览器底部工具栏：`systemUltraThinMaterial`，配合极淡动态边缘与低强度阴影。
...
- 普通/无痕选择位于顶部原生分段控件并与左右分页同步；新建和完成保留在轻量玻璃底部工具栏，不使用通栏蓝色按钮。
