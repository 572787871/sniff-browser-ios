...
### AdGuard Base Filter 广告过滤规则

- 来源：[AdGuard 官方过滤器源](https://filters.adtidy.org/extension/safari/filters/2_optimized.txt)（当前快照，由 [scripts/build-content-blocker.py](scripts/build-content-blocker.py) 生成）
- 内容：EasyList + AdGuard English filter 的 Safari 优化版
- 许可证：GPL-3.0（[AdGuard 过滤器许可证](https://github.com/AdguardTeam/AdguardFilters/blob/master/LICENSE)）
- 使用方式：内置为 `SniffBrowser/Resources/content-blocker-rules.json`，通过 `WKContentRuleList` 在浏览器内执行广告请求过滤。仅保留整域拦截规则（`||domain^`）及其例外，未做其他修改。
